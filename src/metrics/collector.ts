...
export class MetricsCollector {
  ...
  getAverageELU(): number {
    if (this.workerMetrics.size === 0) return 0;
    let sum = 0;
  ...
  getMaxELDp99(): number {
    let max = 0;
    for (const m of this.workerMetrics.values()) {
      if (m.eld.p99 > max) max = m.eld.p99;
    }
    return max;
  }
  ...
  private formatPrometheus(): string {
    ...
    lines.push(
      '# HELP nodejs_eventloop_lag_seconds Avg event loop delay across workers (for K8s HPA)'
    );
