 ...
 */
export async function loadApp(appPath: string): Promise<Server> {
  ...
  if (!create) {
    throw new Error(
      `[volt] App module "${appPath}" must export a create() function that returns an http.Server.\n` +
      `  Supported patterns:\n` +
  ...
  if (!result || typeof result.listen !== 'function') {
    throw new Error(
      `[volt] create() must return an http.Server (or a Promise of one). Got: ${typeof result}`
    );
  }
 ...
 */
function findCreateFunction(mod: any): (() => Server | Promise<Server>) | null {
  // export function create() { ... }
  ...
  if (mod.default) {
    ...
    if (typeof mod.default.create === 'function') return mod.default.create;
  }
