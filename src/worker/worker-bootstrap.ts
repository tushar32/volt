...
import type { WorkerBootstrapData, SupervisorMessage, WorkerMessage } from '../config/config.js';
...
async function bootstrap(): Promise<void> {
  ...
  const log = (msg: string) => console.log(`[volt][worker:${workerId}] ${msg}`);
  const logError = (msg: string, err?: unknown) => console.error(`[volt][worker:${workerId}] ${msg}`, err ?? '');

  // Unhandled error handlers — report to supervisor before exiting
  process.on('uncaughtException', (err) => {
    logError('Uncaught exception', err);
    const msg: WorkerMessage = { type: 'error', workerId, error: String(err) };
  ...
  process.on('unhandledRejection', (reason) => {
    ...
    const msg: WorkerMessage = { type: 'error', workerId, error: String(reason) };
    parentPort!.postMessage(msg);
    process.exit(1);
  });
  ...
  const monitor = new EventLoopMonitor({
    ...
    onSample: (metrics) => {
      ...
      const msg: WorkerMessage = { type: 'metrics', workerId, metrics };
      parentPort!.postMessage(msg);

      // Self-healing: if event loop delay is critically high, restart
      if (metrics.eld.p99 > maxEventLoopLag) {
        logError(
          `Event loop delay critical: p99=${metrics.eld.p99.toFixed(1)}ms (max=${maxEventLoopLag}ms). Requesting restart.`
        );
  ...
  parentPort.on('message', (msg: SupervisorMessage) => {
    switch (msg.type) {
      case 'shutdown':
        log('Received shutdown signal');
        monitor.stop();
        server.close(() => {
          const reply: WorkerMessage = { type: 'shutdown-complete', workerId };
      ...
      case 'ping':
        const pong: WorkerMessage = { type: 'pong', workerId };
  ...
  const ready: WorkerMessage = { type: 'ready', workerId, port };
