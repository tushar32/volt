...
export class Supervisor {
    ...
    constructor(config) {
        ...
        this.healthMonitor = new HealthMonitor(this.workerPool, this.metricsCollector);
    }
    /**
    ...
    async start() {
        if (this.started)
            return;
        console.log(`[volt] Starting supervisor — ${this.config.workers} workers on :${this.config.port}`);
        ...
        if (this.config.healthCheck) {
            ...
            console.log(`[volt] Health checks at http://0.0.0.0:${this.config.healthCheckPort}/health`);
        }
        // 3. Start Prometheus metrics server
        if (this.config.metrics) {
            await this.metricsCollector.startServer(this.config.metricsPort);
            console.log(`[volt] Prometheus metrics at http://0.0.0.0:${this.config.metricsPort}/metrics`);
        }
        // 4. Start periodic health pings
        this.healthMonitor.startPingLoop();
        this.started = true;
        console.log(`[volt] ⚡ Server running at http://${this.config.host}:${this.config.port}`);
    }
    /**
