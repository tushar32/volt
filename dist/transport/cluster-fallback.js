 ...
 */
export async function startWithCluster(config) {
    if (cluster.isPrimary) {
        console.log(`[volt] SO_REUSEPORT not available — using cluster mode (${config.workers} workers)`);
        ...
        cluster.on('exit', (worker, code, signal) => {
            if (code !== 0) {
                console.error(`[volt] Cluster worker ${worker.process.pid} exited (code=${code}, signal=${signal}), restarting...`);
                const id = [...workers.entries()].find(([, w]) => w === worker)?.[0];
                if (id !== undefined) {
        ...
        const create = typeof appModule.create === 'function'
            ? appModule.create
            : typeof appModule.default?.create === 'function'
                ...
                : typeof appModule.default === 'function'
                    ...
                    : null;
        if (!create) {
            console.error(`[volt] Worker ${workerId}: app module must export a create() function`);
            process.exit(1);
        }
        const server = await create();
        server.listen({ port, host }, () => {
            console.log(`[volt] Cluster worker ${workerId} (pid=${process.pid}) listening on ${host}:${port}`);
        });
...
//# sourceMappingURL=cluster-fallback.js.map